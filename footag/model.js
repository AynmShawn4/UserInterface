    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            ...
            if (this.caption != caption){
                this.caption = caption;
                var self = this;
                _.each(self.listeners,function(f){
                    f(self,Date.now());
                });
            }
        },
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            imageModel.addListener(function(imagemodel, eventTime){
                _.each(
                    self.imageModels, function(f,i){
                        if (f.getPath() == imagemodel.getPath()){
                            self.imageModels[i] = imagemodel;
                        }
                    });

            _.each(
                self.listeners, function(f){
                    f( IMAGE_META_DATA_CHANGED_EVENT, self, imageModel, Date.now() );                    
                });
            ...
            _.each(
                this.listeners, function(f){
                    f( IMAGE_ADDED_TO_COLLECTION_EVENT, self, imageModel, Date.now() );                    
                });
        },
        ...
        removeImageModel: function(imageModel) {
            ...
            _.each(
                self.listeners, function(f){
                    f( IMAGE_REMOVED_FROM_COLLECTION_EVENT, self, imageModel, Date.now());                    
                });
